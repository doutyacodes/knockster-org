 ...
 * Get all notifications for security personnel (broadcast) and mark which ones this personnel has read
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const allNotifications = await db
      .select()
      .from(notifications)
      .where(eq(notifications.recipientType, 'security'))
      .orderBy(desc(notifications.sentAt));

    // Get all read statuses for this security personnel
    const readStatuses = await db
      .select()
      .from(notificationReads)
      .where(eq(notificationReads.securityPersonnelId, securityPersonnelId));
