...
/**
 * POST /api/mobile-api/guest/notifications/[id]/read
 * Mark a notification as read for the authenticated guest
 */
export async function POST(
  ...
  try {
    ...
    if (authResult.payload.role !== 'guest') {
      return unauthorizedResponse('Authentication required');
    }

    const guestId = authResult.payload.id;
    const { id: notificationId } = await params;

    // Check if notification exists and is for guests
    const notification = await db
      ...
      .where(
        and(
          ...
          eq(notifications.recipientType, 'guest')
        )
      )
      .limit(1);
    ...
    // Check if already marked as read by this guest
    const existingRead = await db
      ...
      .where(
        and(
          ...
          eq(notificationReads.guestId, guestId)
        )
      )
      .limit(1);
